...
static void
_load_data(Spawny__Server__Data *data, const char *seat);
static void
_free_data(Spawny__Server__Data *data);

static void
_send_message(Spawny__Server__MessageType type,
              Spawny__Server__RequestFeedback *fb,
...
{
   Spawny__Server__RequestFeedback msg =
     SPAWNY__SERVER__REQUEST__FEEDBACK__INIT;
...
#define SAFE_CALL                                                              \
   if (!client_is_greeter(client)) {                                           \
      ERR("Client with pid %d is not allowed to call this",                    \
          client->client_info.pid);                                            \
      client_free(client);                                                     \
      break;                                                                   \
   }
   ...
   switch (msg->type) {
      case SPAWNY__GREETER__MESSAGE__TYPE__HELLO:
         ...
         _send_message(
           SPAWNY__SERVER__MESSAGE__TYPE__DATA_UPDATE, NULL, &system_data, fd);
      ...
      case SPAWNY__GREETER__MESSAGE__TYPE__SESSION_ACTIVATION:
         ...
         if (!session_details(msg->session, NULL, NULL, NULL, NULL)) {
            ...
            server_spawnservice_feedback(
              0, "Activation is called on this session", fd);
            greeter_lockout(seat);
            client_goodbye(client);
         }
         break;
      case SPAWNY__GREETER__MESSAGE__TYPE__LOGIN_TRY:
         ...
         if (client->try) {
            ...
            server_spawnservice_feedback(
              0, "There is already a login try!", fd);
            break;
         }

         template_details_get(msg->login->template_id,
                              &settings.session_desktop,
                              NULL,
                              &settings.session_type);

         settings.session_seat = seat;

         INF("Greeter login try for user %s and template %s",
             msg->login->user,
             msg->login->template_id);
         client->
         try
            = spawnservice_spawn(_session_done,
                                 client,
                                 _session_job,
                                 msg->login->template_id,
                                 _reexecute_greeter,
      ...
      case SPAWNY__GREETER__MESSAGE__TYPE__GREETER_START:
         ...
         if (kill(client->client_info.pid, 0) == -1) {
            ERR("Client %d is not alive but needs to be alive - dropping",
                client->client_info.pid);
            client_free(client);
         } else {
            if (!seat)
...
client_connect(int fd)
{
   ...
   if (getsockopt(fd, SOL_SOCKET, SO_PEERCRED, &client->client_info, &len) !=
         0 ||
       len != sizeof(client->client_info)) {
...
_load_sessions(Spawny__Server__Data *system_data, const char *seat)
{
   ...
   for (unsigned int i = 0; i < number; i++) {
      ...
      if (!session_details(sessions_raw[i],
                           &uid,
                           &sessions[i - offset]->name,
                           &sessions[i - offset]->icon,
                           NULL)) {
         ...
         offset++;
         sessions = realloc(
           sessions, (number - offset) * sizeof(Spawny__Server__Session));
         continue;
      }
...
_load_templates(Spawny__Server__Data *system_data)
{
   ...
   for (unsigned int i = 0; i < number; i++) {
      ...
      template_details_get(templates_raw[i],
                           (const char **)&templates[i]->name,
                           (const char **)&templates[i]->icon,
                           NULL);
   }
