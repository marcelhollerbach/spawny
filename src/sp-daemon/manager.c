...
static void
_fd_set_check(Fd_Register *reg,
              fd_set *set,
              void (*handle)(Fd_Register *reg, int fd),
              int i UNUSED)
{
   // check if set
...
manager_run(void)
{
   while (!stop) {
      ...
      if (result == -1 && !_errno_ignore()) {
         ...
         for (unsigned int i = 0; i < array_len_get(fds); i++) {
            _fd_set_check(
              array_Fd_Register_get(fds, i), &fds_error, _error_available, i);
         }

         // look for read fd´s
         for (unsigned int i = 0; i < array_len_get(fds); i++) {
            _fd_set_check(
              array_Fd_Register_get(fds, i), &fds_read, _read_available, i);
         }
      }
      readout = 0;
...
manager_unregister_fd(int fd)
{
   ...
   // search for the fd
   for (unsigned int field_index = 0; field_index < array_len_get(fds);
        field_index++) {
