...
_field_edit(const char *arg)
{
   ...
   if (!user_db_field_set(username, name, value))
      return false;
...
_field_del(const char *arg)
{
   ...
   if (!user_db_field_del(username, arg))
      return false;
...
main(int argc, char const *argv[])
{
   ...
   for (int i = 1; i < argc; i++) {
      if (!strcmp(argv[i], "--edit")) {
         i++;
         if (!_field_edit(argv[i]))
            return EXIT_FAILURE;
      } else if (!strcmp(argv[i], "--icon")) {
         i++;
         /**FIXME maybe we should import that thing**/
         if (!user_db_field_set(username, "icon", argv[i]))
            return EXIT_FAILURE;
      } else if (!strcmp(argv[i], "--prefered-session")) {
         i++;
         if (!user_db_field_set(username, "prefered-session", argv[i]))
            return EXIT_FAILURE;
      } else if (!strcmp(argv[i], "--del")) {
         i++;
         if (!_field_del(argv[i]))
            return EXIT_FAILURE;
