...
sp_client_session_activate(Sp_Client_Context *ctx, int session)
{
   ...
   Private_Session ps =
     ARRAY_ACCESS(&ctx->private_sessions, Private_Session, session);
                ...
                int template)
{
   ...
   Private_Template pt =
     ARRAY_ACCESS(&ctx->private_templates, Private_Template, template);
...
{
   if (sessions)
      memcpy(sessions, &ctx->sessions, sizeof(Numbered_Array));
   if (templates)
      memcpy(templates, &ctx->templates, sizeof(Numbered_Array));
   if (users)
      memcpy(users, &ctx->users, sizeof(Numbered_Array));
}
...
typedef void (*convert_func)(Sp_Client_Context *ctx,
                             void *data,
                             void *goal,
                             int id);
...
#define FREE_CONVERT(name, type, proto_type, conv, free)                       \
   static void name##s_convert(                                                \
     Sp_Client_Context *ctx UNUSED, void *data, void *goal, int id)            \
   {                                                                           \
      proto_type proto = data;                                                 \
      type name = goal;                                                        \
      conv                                                                     \
   }                                                                           \
   static void name##s_free(void *data)                                        \
   {                                                                           \
      type name = data;                                                        \
      free                                                                     \
   }
...
FREE_CONVERT(
  template,
  Template *,
  Spawny__Server__SessionTemplate *,
  {
     template->id = id;
...
FREE_CONVERT(
  user,
  User *,
  Spawny__Server__User *,
  {
     user->id = id;
     STRDUP_S(proto->icon, user->icon);
     STRDUP_S(proto->name, user->name);
     user->prefered_session =
       _find_simular_session(ctx, proto->prefered_session);
  },
  {
     FREE_S(user->icon);
     FREE_S(user->name);
...
static void
_fill_array(Sp_Client_Context *ctx,
...
#define CONVERT_ARRAY_P(name, data_name, array_type, data_type)                \
   _fill_array(ctx,                                                            \
               ...
               name##_free,                                                    \
               name##_convert)
#define CONVERT_ARRAY(name, array_type, data_type)                             \
   CONVERT_ARRAY_P(name, name, array_type, data_type)
   ...
   CONVERT_ARRAY_P(
     private_sessions, sessions, Private_Session, Spawny__Server__Session *);
   CONVERT_ARRAY_P(private_templates,
                   templates,
                   Private_Template,
                   Spawny__Server__SessionTemplate *);
